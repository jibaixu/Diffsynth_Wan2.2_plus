...
def load_state_dict(file_path, torch_dtype=None, device="cpu", key_filter=None):
    if isinstance(file_path, list):
        state_dict = {}
        for file_path_ in file_path:
            state_dict.update(load_state_dict(file_path_, torch_dtype, device, key_filter=key_filter))
        return state_dict
    if file_path.endswith(".safetensors"):
        return load_state_dict_from_safetensors(file_path, torch_dtype=torch_dtype, device=device, key_filter=key_filter)
    else:
        return load_state_dict_from_bin(file_path, torch_dtype=torch_dtype, device=device, key_filter=key_filter)


def load_state_dict_from_safetensors(file_path, torch_dtype=None, device="cpu", key_filter=None):
    state_dict = {}
...
def load_state_dict_from_bin(file_path, torch_dtype=None, device="cpu", key_filter=None):
