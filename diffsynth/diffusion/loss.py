...
def FlowMatchSFTLoss(pipe: BasePipeline, **inputs):
    ...
    input_latents = inputs["input_latents"] #原始样本
    noise = inputs["noise"]
    ...
    if getattr(pipe, "action_injection_mode", None) == "adaln" and history_t > 1:
        ...
        history_latents = input_latents[:, :, 1:history_t]
        history_noise = noise[:, :, 1:history_t]
        inputs["latents"][:, :, 1:history_t] = pipe.scheduler.add_noise(history_latents, history_noise, small_timestep)
    ...
    for name, tensor in [
        ...
        assert torch.isfinite(tensor).all(), f"{name} has NaN/Inf, min={tensor.min()}, max={tensor.max()}"
